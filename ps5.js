...
showRhymesButton.addEventListener('click',()=>{
    ...
    datamuseRequest(getDatamuseRhymeUrl(), (callback)=>{
        ...
        object = groupBy(rhymeData, 'numSyllables')
        for (let prop in object){
            wordOutput.innerHTML += `<h3> Syllables: ${prop} </h3>`
            for (let i = 0 ; i < object[prop].length ; i++){
                wordOutput.innerHTML += `<li> ${object[prop][i]['word']} <button type="button" class="btn btn-success btn-sm"> (save) </button>  </li>`
            }
...
wordOutput.addEventListener('click', (event)=>{
    if (event.target.classList.contains('btn-success')){
        let word = event.target.parentElement.innerText.split('(')[0]
...
showSynonymsButton.addEventListener('click',()=>{
    outputDescription.innerHTML = ""
    wordOutput.innerHTML = ""
    datamuseRequest(getDatamuseSimilarToUrl(), (callback)=>{
        
        outputDescription.innerHTML = `<h1>Words that similar to : ${wordInput.value}</h1>`;

        const similarData = callback
        if (similarData.length == 0){
            wordOutput.innerHTML+='no result';
            return
        }

        for (let i = 0 ; i < similarData.length ; i++){
             wordOutput.innerHTML += `<li> ${similarData[i]['word']}  <button type="button" class="btn btn-success btn-sm"> (save) </button> </li> `
